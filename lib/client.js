 ...
 */
define(['./xhr', './uri', './resource'], function(xhr, Uri, Resource) {
  ...
  function Client(apiRoot) {
    ...
    this._apiRoot = Uri.parse(apiRoot);
    this._traversals = {};
    this._nodes = {};
  ...
  Client.prototype._getRootDescriptor = function() {
    return this._getDescriptor(this._apiRoot.toString())
    .catch(function() {
  ...
  Client.prototype._isCrossOrigin = function() {
    var domain = window.location.hostname;
    var port = window.location.port;

    return this._apiRoot.isAbsolute() &&
           (domain !== this._apiRoot.host || port !== this._apiRoot.port);
  };
